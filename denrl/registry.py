...
from .env import make_sim_env, PenalizedEnv, ZONE_LOW, ZONE_HIGH
...
def build_env(cfg: dict):
    ...
    `zone` is the (lo, hi) excluded-angle band used by this run — overridable via
    env.excluded_zone (e.g. E3's "shifted danger zone" test), defaults to the
    paper's zone otherwise.
    """
    ...
    zone = tuple(cfg["env"].get("excluded_zone", (ZONE_LOW, ZONE_HIGH)))
