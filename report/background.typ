...
\
Two findings motivate the present study. First, avoidance was not reached within the training budget even at the largest penalty. Second, as the authors note, the weight of the penalty term must be selected according to the environment's reward function. Hence, the original per-step reward in @eq:paper-reward lies in $[-16.27, 0]$, which makes $p = 2$ a small penalty and $p = 30$ nearly twice the worst per-step reward obtainable, so these three values would yield entirely different trade-offs in an environment whose rewards are scaled differently.
...
#v(-10pt)
\
The reference method fixes $w_t equiv p$ at a manually selected constant, leaving the agent to maximize $EE_pi [sum_t gamma^t (r_t - p dot c_(t+1))]$. This is one particular scalarization of return against cost, and the cost level $C(pi_p^*)$ that
...
#v(-10pt)
\
We formulate the requirement as a constraint within a constrained MDP @altman1999, $ max_pi J(pi) quad "subject to" quad C(pi) <= epsilon, $ <eq:cmdp> 

#v(-10pt)
\
in which $epsilon in [0, 1]$ is the largest per-step cost the practitioner is willing to accept. Unlike $p$, the tolerance $epsilon$ specifies an acceptable constraint level independently of the task reward scale. For a binary cost, it represents the maximum permitted fraction of training steps spent in penalized states.
\
...
\
Interpreted as a controller, the update increases the penalty weight when the policy violates the constraint ($hat(C) > epsilon$), increasing the incentive to avoid the penalized region, and lowers it again once the constraint holds. Complementary slackness at a saddle point of @eq:lagrangian requires $alpha^* (C(pi^*) - epsilon) = 0$: 
#v(-10pt)
\
where the constraint is inactive at the optimum, the multiplier is zero and training proceeds on the clean reward, so the penalty deactivates itself when it is not needed. Combining an off-the-shelf actor-critic for the policy step with a slower dual step is a conventional Lagrangian approach in safe RL @tessler2019 @ray2019; @stooke2020 study its transient behaviour and propose PID-controlled variants. We retain the standard form of @eq:dual-ascent, since the study focuses on the effect of learning the penalty weight rather than on tuning the dual controller.
\
