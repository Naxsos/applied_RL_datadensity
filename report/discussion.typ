...
The LunarLander lagr runs used the continuous KDE cost instead of the baselines' binary threshold (@sec-protocol); it is non-zero on $5.6%$ instead of $0.56%$ of offline states and therefore penalizes lateral drift toward the edge of the data coverage earlier than the binary cost does.
...
A meaningful avoidance test requires a zone several bandwidths wide or a smaller bandwidth: on the same offline data, a bandwidth of $0.02$ assigns cost $1$ to about $43%$ of the box while raising the share of penalized offline states only from $0.56%$ to $0.8%$ (same 5,000-state sample as in @sec-protocol).
